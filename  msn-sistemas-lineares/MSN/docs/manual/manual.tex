...
O método de Gauss-Jacobi fundamenta-se a resolver a equação:

\begin{equation*}
Ax = b
\end{equation*}

Sabendo que a matriz A pode ser escrita na forma 

\begin{equation*}
A = D + (L + U)
\end{equation*}

onde D representa a diagonal, L a matriz estritamente inferior e U a matriz estritamente superior. Substituindo o resultado na equação e isolando a variável x no primeiro membro, temos que:
...
Há alguns casos onde o método converge não satisfazendo a condição anterior, sendo necessário que os elementos na diagonal principal sejam maiores em magnitude que os outros elementos.
