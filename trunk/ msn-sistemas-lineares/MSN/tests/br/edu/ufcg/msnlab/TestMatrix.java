...
**/
      ...
      B = new Matrix(bvals);
      System.out.println(B.norm2()+" maior valor B");
      Chol = A.chol(); 
      L = Chol.getL();
      try {
         check(A,L.times(L.transpose()));
      ...
      X.print(3, 5);
      System.out.println(X.norm2()+" maior valor X");
      //Fim do Segundo teste de Cholesky
 ...
 /*     System.out.println("Primeiro teste de CholeskySolverImpl");
    //Primeiro teste de CholeskySolverImpl
      double[][] hvals = {{1.,1.,0.},{1.,2.,-1.},{0.,-1.,3.}};

	    List<Double> termos = new ArrayList<Double>();
	    termos.add(2.);
	    termos.add(1.);
	    termos.add(5.);

 
	    CholeskySolverImpl<Result> ch = new CholeskySolverImpl<Result>();
	   ch.solve(hvals, termos, 0.000005, 50000);
	   System.out.println("Segundo teste de CholeskySolverImpl");
	 //Segundo teste de CholeskySolverImpl
	   
	   double[][] hvals1 = {{4.,2.,-4.},{2.,10.,4.},{-4.,4.,9.}};

	    List<Double> termos1 = new ArrayList<Double>();
	    termos1.add(2.);
	    termos1.add(16.);
	    termos1.add(9.);

	   ch.solve(hvals1, termos1, 0.000005, 50000);
	   
	   System.out.println("Terceiro teste de CholeskySolverImpl");
	   //Terceiro teste de CholeskySolverImpl
	   //Exemplo que a matriz naum eh simetrica positiva
	   
	   double[][] hvals2 = {{3.,1.,0.},{1.,3.,2.},{0.,2.,0.}};

	    List<Double> termos2 = new ArrayList<Double>();
	    termos2.add(2.);
	    termos2.add(16.);
	    termos2.add(9.);

	   ch.solve(hvals2, termos2, 0.000005, 50000);
	  */ 
  ...
  /** Print appropriate messages for unsuccessful outcome try **/
   ...
   private static void print(double[][] x1, int w, int d) {
      // Use format Fw.d for all elements.
      System.out.print("\n");
      new Matrix(x1).print(w,d);
      print("\n");
   }
